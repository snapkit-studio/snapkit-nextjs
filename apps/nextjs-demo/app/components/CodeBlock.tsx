...
export function CodeBlock({ children, language = "tsx", className = "" }: CodeBlockProps) {
  ...
  if (isLoading) {
    return (
      <pre className={`animate-pulse overflow-x-auto rounded-lg bg-gray-900 p-4 text-sm text-gray-100 ${className}`}>
        <code>{children}</code>
      </pre>
    );
  }

  return <div className={`shiki-container ${className}`} dangerouslySetInnerHTML={{ __html: highlightedCode }} />;
}
